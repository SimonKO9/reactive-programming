...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      ...
      case head :: tail =>
        head.flatMap { value =>
          all(tail).flatMap(x => Future(value :: x))
        }
    }
    ...
    def any[T](fs: List[Future[T]]): Future[T] = {
      ...
      fs.foreach { f =>
        if (!p.isCompleted) f.onComplete(value => Try(p.complete(value)))
      }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    /** Returns the result of this future if it is completed now.
      ...
      */
    def now: T = Try(Await.result(f, 0 seconds)) match {
