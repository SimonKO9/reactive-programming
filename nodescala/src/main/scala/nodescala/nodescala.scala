...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    ...
    val nextRequestSubscription = Future.run() { token =>
      async {
        while (!token.isCancelled) {
          listener.nextRequest().onSuccess {
            case (request, exchange) =>
              val responseFuture = Future(respond(exchange, token, handler(request)))
              val timeoutFuture = Future.delay(50.milliseconds)

              Future.any(List(responseFuture, timeoutFuture)).onComplete {
                case _ => exchange.close()
              }
          }
        }
      }
    ...
    Subscription(listenerSubscription, nextRequestSubscription)
  }
