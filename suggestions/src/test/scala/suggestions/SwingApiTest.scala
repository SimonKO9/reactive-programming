...
class SwingApiTest extends FunSuite {
  ...
  test("SwingApi should stop emitting events after unsubscribing") {
    val textField = new swingApi.TextField
    val values = textField.textValues
    ...
    assert(observed === Seq("H", "Hi"), observed)

    sub.unsubscribe()
    textField.text = "Hip"
    assert(sub.isUnsubscribed, observed)
    assert(observed === Seq("H", "Hi"), observed)
  }
  ...
  test("SwingApi should emit button clicks to the observable") {
    ...
    assert(observed.size === 2, observed)
  }

  test("SwingApi should stop emitting button clicks to the observable after unsubscribing") {
    val button = new swingApi.Button
    val clicks = button.clicks
