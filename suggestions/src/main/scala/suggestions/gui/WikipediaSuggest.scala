...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    val selections: Observable[String] = button.clicks.flatMap { _ =>
      val suggestionTextList: List[String] = suggestionList.selection.items.toList
      Observable.from(suggestionTextList)
    }
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(pageText) => editorpane.text = pageText
        case Failure(ex) => status.text = ex.getMessage
      }
    }
